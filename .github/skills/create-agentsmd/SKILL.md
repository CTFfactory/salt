...
argument-hint: '[repository context or blank for auto-discovery]'
---
...
1. **Analyze the project structure** to understand:
   ...
   - Package managers and build tools (Makefile, CMake, APT, Homebrew)
   - Testing frameworks and coverage tooling
   - Project architecture (monorepo, single package, etc.)
   - Build system entry point and primary toolchain version

2. **Identify key workflows** by examining:

   - Makefile targets (`make help` output or target annotations)
   - Makefile for build targets and primary toolchain version
   - CI/CD configuration files (.github/workflows/)
   - Release workflow and Makefile release targets for release patterns
   - Man pages for CLI documentation

3. **Create comprehensive sections** covering:

   - All essential setup and development commands
   - Testing strategies and commands (unit, integration, benchmarks)
   - Code style and conventions (linters, formatters)
   - Build and deployment processes (Makefile release targets, Docker builds, APT packaging)

4. **Include specific, actionable commands** that agents can execute directly

5. **For monorepos**, create:
   - A root AGENTS.md covering the full project
    - Per-tool AGENTS.md files in `docs/<tool>/AGENTS.md` covering tool-specific details
   - The closest AGENTS.md file takes precedence for any given location

6. **Test the instructions** by ensuring all commands work as documented

7. **Keep it focused** on what agents need to know, not general project information
...
- **Consider nested files**: For monorepos, create AGENTS.md files in subproject directories
- **List Make targets**: Extract and document all `make` targets with descriptions
- **Include build entry point**: Always state the build system and primary toolchain requirements (e.g., C standard, compiler, key library versions)
- **Document architecture**: Show the directory tree with brief annotations
- **Cross-reference tools**: In monorepos, list all tools with their entrypoints and purposes
