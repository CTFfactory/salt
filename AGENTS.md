...
|---|---|---|
| Language | C17 | Production CLI and library entrypoints |
...
- `.github/workflows/release.yml` runs the full CI gate, builds the Linux amd64
  release archive (with static `salt` and dynamic `salt-dynamic`), and publishes
  `SHA256SUMS` plus `SHA256SUMS.asc` on semver tags.
- `.github/workflows/codeql.yml` provides additional code analysis coverage.
- `docker/ubuntu/{22,24,26}/Dockerfile` doubles as the supported Ubuntu
