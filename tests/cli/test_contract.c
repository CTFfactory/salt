...
void test_salt_cli_run_malformed_argv_rejected(void **state) {
    char *argv_empty_program[] = {(char *)"", (char *)"--help"};
    char *argv_missing_program[] = {NULL, (char *)"--help"};
    char *argv_with_hole[] = {(char *)"salt", NULL};
    FILE *in_stream = stream_from_text(NULL);
    FILE *out_stream = test_tmpfile_checked();
    FILE *err_stream = test_tmpfile_checked();
    ...
    assert_int_equal(
        salt_cli_run_with_streams(2, argv_empty_program, in_stream, out_stream, err_stream),
        SALT_ERR_INPUT);
    assert_int_equal(
        salt_cli_run_with_streams(2, argv_missing_program, in_stream, out_stream, err_stream),
        SALT_ERR_INPUT);
    assert_int_equal(
        salt_cli_run_with_streams(2, argv_with_hole, in_stream, out_stream, err_stream),
        SALT_ERR_INPUT);
...
void test_salt_cli_key_id_option_too_long(void **state) {
    char key_id[SALT_MAX_KEY_ID_LEN + 2U];
    char *argv[] = {(char *)"salt",     (char *)"--key", (char *)"base64-key",
                    (char *)"--key-id", key_id,          (char *)"hello"};
    FILE *in_stream = stream_from_text(NULL);
    FILE *out_stream = test_tmpfile_checked();
    FILE *err_stream = test_tmpfile_checked();
    char stdout_buffer[64];
    ...
    /* Annex K memset_s is unavailable on glibc; size is bounded by key_id array. */
    /* NOLINTNEXTLINE(clang-analyzer-security.insecureAPI.DeprecatedOrUnsafeBufferHandling) */
    memset(key_id, 'k', sizeof(key_id) - 1U);
    key_id[sizeof(key_id) - 1U] = '\0';
    ...
    read_stream(err_stream, stderr_buffer, sizeof(stderr_buffer));
    assert_string_equal(stdout_buffer, "");
    assert_string_equal(stderr_buffer, "error: --key-id/-i exceeds maximum length (256 bytes)\n");
...
int run_cli_contract_tests(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_salt_cli_run_null_arguments_rejected, cli_test_setup,
                                        cli_test_teardown),
        cmocka_unit_test_setup_teardown(test_salt_cli_run_malformed_argv_rejected, cli_test_setup,
                                        cli_test_teardown),
        cmocka_unit_test_setup_teardown(test_salt_cli_key_id_option_too_long, cli_test_setup,
                                        cli_test_teardown),
