...
/*
 * JSON escape sequence table for RFC 8259 compliance.
 * Maps escape characters (after backslash) to their unescaped byte values.
 * This table implements CERT STR02-C: avoid magic numbers by centralizing
 * escape sequence definitions for maintainability and testability.
 */
static const struct {
    char in;  /* Escape character (the character after the backslash) */
    char out; /* Unescaped byte value */
} ESCAPE_TABLE[] = {
    {'n', '\n'},  /* Newline (U+000A) */
    ...
    {'/', '/'},   /* Solidus (U+002F), allowed for compatibility */
};

static const size_t ESCAPE_TABLE_LEN = sizeof(ESCAPE_TABLE) / sizeof(ESCAPE_TABLE[0]);

/*
 * Lookup an escape sequence in the escape table.
 * Returns true and sets out_value if found; returns false if not found.
 */
static bool escape_lookup(char escape_char, char *out_value) {
    for (size_t j = 0U; j < ESCAPE_TABLE_LEN; ++j) {
        if (ESCAPE_TABLE[j].in == escape_char) {
            *out_value = ESCAPE_TABLE[j].out;
            return true;
        }
    }
    return false;
...
static bool json_parse_string_token(const char **cursor, char *out, size_t out_size) {
    ...
    while (**cursor != '\0' && **cursor != '"') {
        ...
        if (**cursor == '\\') {
            ...
            /* Table lookup for escape sequences (RFC 8259 compliant) */
            if (!escape_lookup(**cursor, &escape_value)) {
                /* escape_char not found in table: reject unsupported escapes (\u, \b, \f, etc.) */
                return false;
            }
    ...
    /* CWE-119/125: Explicit bounds check before null-terminator write (EXP34-C) */
    if (i >= out_size) {
        return false;
    }
