...
int salt_cli_prepare_output(FILE *err_stream, struct salt_cli_state *state) {
    ...
    /*
     * Best-effort lock of the output buffer to minimize swap exposure.
     *
     * The output buffer will hold base64-encoded ciphertext after encryption, which
     * is not directly sensitive but may contain ephemeral cryptographic material
     * (sealed-box overhead) that benefits from locking to reduce forensic recovery risk.
     *
     * Failure is non-fatal: sodium_munlock() still wipes the buffer in cleanup,
     * preserving MEM03-C compliance (sensitive-buffer zeroing). Best practices:
     * - High-security environments should increase ulimit -l before invoking the CLI.
     * - Containers may need mlock capability disabled; in such cases, zeroing on
     *   cleanup still provides the security invariant.
     */
