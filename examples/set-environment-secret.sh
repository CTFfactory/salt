...
#   GITHUB_TOKEN      - GitHub token with repository Environments:write permission
#   OWNER             - Repository owner
#   REPO              - Repository name
#   ENVIRONMENT_NAME  - Environment name (will be URL-encoded)
...
ENC_ENVIRONMENT_NAME="$(printf '%s' "${ENVIRONMENT_NAME}" | jq -sRr @uri)"
...
KEY_JSON="$(fetch_public_key_json \
  "https://api.github.com/repos/${OWNER}/${REPO}/environments/${ENC_ENVIRONMENT_NAME}/secrets/public-key" \
  "environment")"
...
if [ -z "${KEY_ID}" ] || [ "${KEY_ID}" = "null" ] || [ -z "${PUB_KEY}" ] || [ "${PUB_KEY}" = "null" ]; then
  printf 'failed to retrieve key_id or key from environment public key response\n' >&2
  exit 1
fi
...
STATUS="$(printf '%s' "${SALT_JSON}" | jq -c '{encrypted_value:.encrypted_value,key_id:.key_id}' | \
  curl -sS -L -o /dev/null -w '%{http_code}' \
    ...
    -H @<(auth_header_fd) \
    -H "X-GitHub-Api-Version: ${GITHUB_API_VERSION}" \
    "https://api.github.com/repos/${OWNER}/${REPO}/environments/${ENC_ENVIRONMENT_NAME}/secrets/${SECRET_NAME}" \
    --data-binary @-)"

if [ "${STATUS}" = "201" ] || [ "${STATUS}" = "204" ]; then
  printf 'environment secret %s applied\n' "${SECRET_NAME}"
else
  printf 'environment secret apply failed, status=%s\n' "${STATUS}" >&2
  exit 1
fi
